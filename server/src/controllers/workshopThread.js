...
const threads_db = require('../db/workshopThread.js');
...
async function get_critiques_from_thread(req, res) {
    const params = Object.values(req.params);
    const results = await threads_db.get_critiques_from_thread(params);
    res.send(results);
}
